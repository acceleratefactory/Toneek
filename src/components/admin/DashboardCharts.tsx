...
export default function DashboardCharts({ historicalOrders }: { historicalOrders: any[] }) {
  // ── Calculate Derived Data (Fallback to logical placeholders if empty) ──
  const rawMonths = ['Oct', 'Nov', 'Dec', 'Jan', 'Feb', 'Mar', 'Apr']
  
  // Bar Chart Data (Revenue)
  const revenueData = rawMonths.map(month => ({
    name: month,
    revenue: month === 'Apr' ? historicalOrders.reduce((sum, o) => sum + (o.payment_amount || 0), 0) : Math.floor(Math.random() * 500) + 200,
    cost: month === 'Apr' ? 120 : Math.floor(Math.random() * 200) + 50
  }))

  // Line Chart Data (Subscriptions Growth)
  const lineData = rawMonths.map((month, idx) => ({
    name: month,
    total: month === 'Apr' ? historicalOrders.length + 5 : idx * 2 + 1
  }))

  // Donut Chart Data (Plans)
  const totalOrders = Math.max(historicalOrders.length, 1) // prevent div by zero
  const fullProto = historicalOrders.filter(o => o.plan_tier === 'full_protocol').length || 1
  const starterProto = historicalOrders.filter(o => o.plan_tier === 'starter_routine').length || 0
  const percentages = { full: Math.round((fullProto/totalOrders)*100), starter: Math.round((starterProto/totalOrders)*100) }
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col overflow-hidden relative">
         ...
         <div className="p-8 flex gap-8">
            ...
            <div className="flex-1 relative h-64 border-l border-b border-gray-200 p-0" onMouseLeave={() => setActiveLineItem(null)}>
               {/* Y Axis Guides */}
               <div className="absolute w-full border-b border-gray-100 border-dashed left-0 bottom-[25%] -z-10 flex items-center justify-between"><span className="absolute -left-8 text-xs text-gray-400 font-bold">50</span></div>
               <div className="absolute w-full border-b border-gray-100 border-dashed left-0 bottom-[50%] -z-10 flex items-center justify-between"><span className="absolute -left-8 text-xs text-gray-400 font-bold">100</span></div>
               <div className="absolute w-full border-b border-gray-100 border-dashed left-0 bottom-[75%] -z-10 flex items-center justify-between"><span className="absolute -left-8 text-xs text-gray-400 font-bold">150</span></div>
               ...
               <svg className="absolute inset-0 w-full h-full overflow-visible pointer-events-none" preserveAspectRatio="none">
                 <path 
                   d={`M 0,${256 - (lineData[0].total/200)*256} ${lineData.map((d, i) => `L ${i * (100/(lineData.length-1))}%, ${256 - (d.total/200)*256}`).join(' ')}`}
                   fill="none" 
                   stroke="#3b82f6" 
                   strokeWidth="2"
                 />
                 ...
                 <path 
                   d={`M 0,256 L 0,${256 - (lineData[0].total/200)*256} ${lineData.map((d, i) => `L ${i * (100/(lineData.length-1))}%, ${256 - (d.total/200)*256}`).join(' ')} L 100%, 256 Z`}
               ...
               <div className="absolute w-full h-full flex justify-between items-end">
                 {lineData.map((d, i) => (
                    ...
                    >
                      ...
                      {/* Glowing Dot overlay */}
                      <div className="absolute w-3 h-3 bg-[#3b82f6] border-2 border-white rounded-full shadow-md z-10 transition-transform group-hover:scale-150" style={{ bottom: `calc(${(d.total/200)*100}%)`, transform: 'translateY(50%)' }}></div>
                    </div>
                 ))}
               </div>
            ...
            <div className="w-64 flex flex-col justify-center border-l border-gray-100 pl-8 space-y-6">
              <div>
                 <div className="flex items-center gap-2 mb-1">
                   ...
                   <span className="text-xs text-gray-500 font-semibold uppercase">Subs as of {rawMonths[0]} 2026</span>
                 </div>
              ...
              <div className="pt-4 border-t border-gray-100 mt-2">
                 <div className="flex items-center gap-2 mb-1">
                   ...
                   <span className="text-xs text-gray-500 font-semibold uppercase">Subs as of {rawMonths[rawMonths.length-1]} 2026</span>
                 </div>
